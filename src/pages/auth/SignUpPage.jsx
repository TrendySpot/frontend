...
// 백엔드 RegisterRequest: email, password, nickname, termsAgreed
// 비밀번호: 8자 이상, 소문자+숫자+특수문자 (대문자 선택)
...
const SignUpPage = () => {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      setErrors((p) => ({ ...p, email: err.response?.data?.message ?? "발송 실패" }));
      ...
      <form className="auth-card" onSubmit={handleSubmit} style={{ width: 460, gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <input className="input-field" name="email" type="email"
              ...
              disabled={verified} style={{ flex: 1 }} />
            {!verified && (
        ...
        {codeSent && !verified && (
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input className="input-field" placeholder="인증 코드 6자리"
        ...
        {pwStrength && (
          <div style={{ marginTop: -8 }}>
            <div style={{ height: 4, background: "#f1f2f6", borderRadius: 999, overflow: "hidden" }}>
              <div style={{ height: "100%", width: pwStrength.width,
                            background: pwStrength.color, borderRadius: 999, transition: "0.3s" }} />
            </div>
        ...
        <div>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 14 }}>
            <input type="checkbox" name="termsAgreed" checked={form.termsAgreed}
              onChange={handleChange}
