    ...
    >
      ...
      >
        ...
        <div style={{ position: "relative" }}>
          <img
            src={ticket.imageUrl || "https://images.unsplash.com/photo-1545987796-200677ee1011?w=420"}
        ...
        <div style={{ padding: "20px 24px 24px" }}>
          <h2 style={{ fontSize: 18, fontWeight: 900, margin: "0 0 16px", color: "#1e1e2f" }}>
            {ticket.spotTitle}
          </h2>

          {[
            ["🎟 티켓 번호", `#${ticket.ticketId}`],
            ["📅 방문일", ticket.eventDate ? dayjs(ticket.eventDate).format("YYYY년 MM월 DD일 (ddd)") : "-"],
            ["👥 인원", `${ticket.ticketCount}명`],
            ["💳 결제 금액", ticket.price === 0 ? "무료" : ticket.price ? `₩${ticket.price.toLocaleString()}` : "-"],
            ["🕐 예약일", ticket.createdAt ? dayjs(ticket.createdAt).format("YYYY.MM.DD HH:mm") : "-"],
          ].map(([label, value]) => (
            <div key={label} style={{
              display: "flex", justifyContent: "space-between",
              padding: "10px 0", borderBottom: "1px solid #f1f2f6",
              fontSize: 14,
            }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                background: "#f7f8fc", borderRadius: 20,
            ...
            >
              <img
                src={t.imageUrl || "https://images.unsplash.com/photo-1545987796-200677ee1011?w=80"}
                alt={t.spotTitle}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: 12, color: "#6b7280", margin: "0 0 2px" }}>
                  방문일: {t.eventDate ? dayjs(t.eventDate).format("YYYY년 MM월 DD일") : "-"}
                </p>
                <p style={{ fontSize: 12, color: "#6b7280", margin: "0 0 2px" }}>인원: {t.ticketCount}명</p>
                <p style={{ fontSize: 13, fontWeight: 700, margin: 0 }}>
                  {t.price === 0 ? "무료" : t.price ? `₩${t.price.toLocaleString()}` : "-"}
                </p>
              </div>
            </div>
          );
