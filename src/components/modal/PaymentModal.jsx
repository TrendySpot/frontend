...
const PaymentModal = ({ isOpen, onClose, spot, reservation, onSuccess }) => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (!PortOne) {
        throw new Error("포트원 SDK가 로드되지 않았습니다. 잠시 후 다시 시도해주세요.");
      }
      ...
      <div className="modal-box" style={{ maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          ...
          <button onClick={onClose}
            style={{ border: 0, background: "transparent", cursor: "pointer", fontSize: 20, color: "#6b7280" }}>
            ✕
          </button>
        </div>
        <div className="modal-body" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "#f7f8fc", borderRadius: 16, padding: "16px 20px",
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center",
                        paddingTop: 12, borderTop: "1px solid #f1f2f6" }}>
